...
public class GraphExtractor {
    ...
    long edgeCount = 0;
    char keySepChar = '_';
    static List<File> lastFiles(int datNumberId) {
    ...
    public Graph extractGraphFor(ZonedDateTime historyStartsFrom, ZonedDateTime historyEndsAt, int datFileHint) throws SQLException, ClassNotFoundException {
        ...
        for (Block block : loader) {
            ...
            LocalDateTime blockTime = block.getTime().toInstant()
                    ...
                    .toLocalDateTime();
            if (historyStartsFrom != null && blockTime.compareTo(ChronoLocalDateTime.from(historyStartsFrom)) < 0) {
                //we do not care about earlier blocks
                continue;
            }
            if (blockTime.compareTo(ChronoLocalDateTime.from(historyEndsAt)) > 0) {
                continue;
            }
